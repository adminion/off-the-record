    ...
    function friendshipPlugin (schema) {
        ...
        schema.static({
            ...
            friendRequest : function (requesterId, requestedEmail, done) {
                ...
                this.viewProfile(requesterId, requestedEmail, function (err, accountInfo) {
                    if (err) {
                        done(err);
                    } else if (!accountInfo) {
                        done();
                        ...
                        var conditions = {
                            '$or': [
                                { requester: requesterId,       requested: accountInfo._id  },
                                { requester: accountInfo._id,   requested: requesterId      }
                            ]
                        };
                        ...
                        // get the friendship itself
                        self.getFriendship(requesterId, accountInfo._id, function (err, friendship) {
                            if (err) {
                                done(err);
                            } else if (friendship) {
                                ...
                                if (!accountInfo.friendRequests) {
                                    done(new Error('You are not allowed to send this user a friendRequest.'));
                                    debug('friend request no allowed!');
                                } else {
                                    var request = new Friendship({ 
                                        requester: requesterId,
                                        requested: accountInfo._id
