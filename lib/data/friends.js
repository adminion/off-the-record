...
module.exports = function (accountRef) {
    ...
    function friendshipPlugin (schema) {
        ...
        var settingDef = { 
            type: Number, 
            min: privacy.values.NOBODY, 
            max: privacy.values.ANYBODY, 
        ...
        schema.static({
             ...
             *  Model.viewProfile()
             */
            viewProfile: function (accountId1, accountId2, done) {
                var self = this,
                    model = mongoose.model(accountRef, schema);

                this.getRelationship(accountId1, accountId2, function (err, relationshipValue) {

                    debug('relationshipValue', relationshipValue);

                    model.findById(accountId2, function (err, account) {
                        debug('account', account);

                        var accountInfo = {};
                        var privProperties = ['profile', 'friendRequests', 'chatRequests'];

                        // add things depending on relationship and privacy settings
                        for (var i = 0; i < privProperties.length; i++) {
                            debug('privProperties[%s]', privProperties[i]);

                            var privProperty = privProperties[i];

                            // the value of the 
                            var propertyValue = account.privacy[privProperty]
                            debug('propertyValue', propertyValue);

                            if (propertyValue !== privacy.values.NOBODY) {
                                debug('propertyValue !== privacy.values.NOBODY');
        ...
        schema.method({
             ...
             */
            viewProfile : function (accountId, done) {
                schema.statics.viewProfile(this._id, accountId, done);
            }
