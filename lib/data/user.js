...
UserSchema.statics.search = function (user, term, options, searchComplete) {
    ...
    // get the user's friends, friendsOfFriends, pendingFriends, and nonFriends
    async.parallel({
        ...
        pendingFriends: function (done) {
            user.getPendingFriends(function (err, pendingFriends) {
                async.filter(pendingFriends, function (pendingFriend, filtered) {
                    // pendingFriend.allows(user, 'profile', function (err, answer) {
                    //     if (err) return done(err);
                        // filtered(answer);
                    // });
                    filtered(true);
                }, function (pendingFriends) {
                    debug('pendingFriends', pendingFriends);
                    done(null, pendingFriends);
                });
            });
        },
        friendsOfFriends: function (done) {
            user.getFriendsOfFriends(done);
        },
...
UserSchema.statics.viewProfile = function (viewer, viewee, done) {
    ...
    async.parallel({
        friendship: function (done) {
            self.getFriendship(viewer._id, viewee._id, done);
        },
        relationship: function (done) {
            self.getRelationship(viewer._id, viewee._id, done);
        },
        profile: function (done) {
            viewee.allows(viewer._id, 'profile', done);
        },
        friendRequest: function (done) {
            viewee.allows(viewer._id, 'friendRequest', done);
        },
        startConversation: function (done) {
            viewee.allows(viewer._id, 'startConversation', done);
        }
    }, function (err, results) {
        if (err) return done(err);
        ...
        // chat requests may be sent if the viewee's privacy allows
        userInfo.startConversation = (results.relationship >= viewee.privacy.friendRequest);
