...
var convoRef =  { type: ObjectId, ref: options.conversationModelName };
...
UserSchema.statics.search = function (searcher, findParams, searchComplete) {
  ...
  async.parallel({
    ...
    //   ], function (err, users, permissions) {
    //     if (err) return done(err);
    //     debug('users', users);
    //     debug('permissions', permissions);
    //     done(null, {users: users, permissions: permissions});
    //   });
    // },
...
UserSchema.statics.viewProfile = function (requester, requestee, done) {
  ...
  requestee.getPermissions(requester, function (err, permissions) {
