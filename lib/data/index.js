...
function OffTheRecord_data () {
    ...
    this.search = function (user, term, done) {
        ...
        var conditions = {
            '$or': [{displayName: term}, { firstName: term}, {lastName: term}],
            'privacy.search' : { '$gte' : env.privacy.FRIENDS }
        };
        ...
        Accounts.model.getFriends(user._id, function (err, friends) {
            ...
            if (friends.length > 0) {
                ...
                friends.forEach(function (friend) {
                    ...
                    conditions['privacy.search'] = { '$gte': env.privacy.FRIENDS_OF_FRIENDS }

                    // search each friends of each friend
        ...
        function nonFriends () {
            conditions['privacy.search'] = { '$gte' : env.privacy.ANYBODY };
            debug('conditions', conditions)          

            Accounts.model.find(conditions, options, function (err, nonFriends) {
                if (err) {
                    done(err);
                }
