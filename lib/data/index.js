...
function OffTheRecord_data () {
    ...
    function init () {
        ...
        passport.use(self.users.createStrategy());
        // serializeUser() and deserializeUser() return the functions passport will use
        passport.serializeUser(self.users.serializeUser());
        passport.deserializeUser(self.users.deserializeUser());

        // debug('self.users', self.users);
