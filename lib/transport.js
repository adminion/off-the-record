...
function Offtherecord_transport (data) {
  ...
  function startHttp () {
    ...
    function cachedResponse (request, response) {
      ...
      // debug('cachedResponse', cachedResponse);
      ...
      //     "Content-Length": Buffer.byteLength(cachedResponse)
      //   }
      // );
  ...
  function startRealtime () {
    ...
    function socketConnect (socket) {
      ...
      if (!state.online.has(socket.user.id)) {
        ...
        publish(socket.user.id, 'friends:login', socket.user.username);
      ...
      function socketDisconnect () {
        ...
        if (user.sockets.size === 0) {
          ...
          user.logoutTimeout = setTimeout(function logoutTimeout () {
            // logout the user
