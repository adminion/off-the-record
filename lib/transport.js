...
function Offtherecord_transport (data) {
    ...
    async.parallel({
      ...
      cachedResponses: function (done) {
        ...
        async.each(routesToCache, function (route, complete) {
          ...
          fs.readFile(__dirname + '/html/' + route + '.html', function (err, fileBuffer) {
    ...
    function socketConnect (socket) {
      ...
      function convosStart (invitees, done) {
        ...
        invitees.forEach(function (invitee, key) {
          ...
          invitee = state.users.byUserName.get(invitee);

          var allowed = state.getPermissions(socket.user.id, invitee.id).has('startConversation');
          ...
          if (allowed) {
            ...
            if (!allowed) {
              ...
              debug('state.users[%s]: %s', invitee.username, invitee);
  ...
  function getClientConvo (convoId) {
    ...
    convo.invitees.forEach(function (invitee) {
      clientConvo.invitees.push(state.users.get(invitee.toString()).username)
    });
