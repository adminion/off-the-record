...
function State () {
    ...
    value: (user) => {
      ...
      Object.defineProperty(user.friends, 'online', {
        value: () => {
          ...
          return Array.from(user.friends).filter((friend) => {
  ...
  Object.defineProperty(this.sockets, 'connect', {
    value: (userId, socket) => {
      ...
      if (!user.sockets.size) {
        ...
        if (user.logoffTimeout) {
          // cancel pending logoff
          clearTimeout(user.logoffTimeout);
          user.logoffTimeout = undefined;
        }
      }
  ...
  this.getRelationship = function (userId1, userId2) {
    ...
    for (var requestsEntry of user1.friends) {
    ...
    for (var friendsEntry of user2.friends) {
      for (var friendsOfFriendsEntry of friendsEntry[1].friends) {
