...
$(document).ready(function () {
  ...
  client.once('ready', function clientInit () {
    ...
    searchUsers.on('vclick', function searchUsers_onclick (clickEvent) {
  ...
  function searchResults (err, results) {
    ...
    for (var category in results) {
      ...
      results[category].users.forEach(function (username) {
        ...
        var permissions = results[category].permissions[username];

        var result = $('<li></li>');

        // if the searcher is not consented to view this user's profile, just display their username
        
        var usernameLink = (~permissions.indexOf('profile')) 
        ...
        permissions.forEach(function (permission) {
          ...
          if (permission !== 'search' && permission !== 'profile') {
            ...
            interactionButton.on('vclick', function (clickEvent) {
              ...
              client[permission](username, function (err, request) {
                if (err) {
  ...
  function switchView (path) {
    ...
    if (!currentView) {
      $('#' + targetView).fadeOut(100, function () {
        $('#' + targetView).fadeIn(100, 'swing', viewFadedIn);
      });
    } else {
    ...
    function viewFadedIn () {
      // if (targetView in viewFocus) {
      //   viewFocus[targetView](targetItemId);
      // }
    }
  };
