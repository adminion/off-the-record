...
function OffTheRecord_Server (options) {
  ...
  var options = utils.extend(config, options);
  ...
  function startData () {
    ...
    self.data.once('started', startHTTP);
  ...
  function startRealtime () {
    ...
    function socketConnect (socket) {
      ...
      function convosStart (invitees, done) {
        ...
        var convo = self.data.state.convos.add(new self.data.conversation({
      ...
      function friendsPermissions (done) {
        ...
        socket.user.requests.sent.forEach((requested) => {
          permissions.sent[requested.username] = Array.from(self.data.state.getPermissions(sock.user.id, requested.id));
        });

        socket.user.requests.received.forEach((requester) => {
          permissions.received[requester.username] = Array.from(self.data.state.getPermissions(sock.user.id, requester.id));
        });        
  ...
  function getClientUser (userId) {
    ...
    user.friends.forEach((friend) => {
      ...
      clientUser.permissions[friend.username] = Array.from(self.data.state.getPermissions(userId, friend.id));
    });
    ...
    user.requests.sent.forEach((user) => {
      ...
      if (!(user.username in clientUser.permissions) ) {
        ...
        clientUser.permissions[user.username] = Array.from(self.data.state.getPermissions(userId, user.id));
      };
    });
    ...
    user.convos.forEach((convo) => {
      ...
      convo.members.forEach((member) => {
        ...
        if (!(member.username in clientUser.permissions) ) {
          ...
          clientUser.permissions[member.username] = Array.from(self.data.state.getPermissions(userId, member.id));
        };
      });
    });
