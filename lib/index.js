...
function OffTheRecord_Server (options) {
  ...
  let delimiter = this.env.serverName + '$';
  ...
  this.once('started', function () {
    ...
    // check the arguments for -i or --interactive
    if (~process.argv.indexOf('-i') || ~process.argv.indexOf('--interactive')) {
      // if found, show the CLI prompt
      cli.show();
    }
  ...
  this.once('stopped', function () {
    // nothing needed right now... I think..
  });
