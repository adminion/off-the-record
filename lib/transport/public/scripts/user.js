...
$(document).ready(function documentReady () {
  ...
  client.once('ready', function clientReady() {
    ...
    client.viewProfile(username, profileReceived);

    function profileReceived (err, userInfo) {
      
      if (err) return console.error(err); 

      $('#username-heading').empty().append(username);
        ...
        if (consentedInteraction !== 'search' && consentedInteraction !== 'profile') {
          ...
          interactionButton.click(function (clickEvent) {
            ...
            if (consentedInteraction === 'startConversation') {
              client.startConversation([username], function (err, conversation) {
                window.location.href = '/conversations/' + conversation._id
              });
            } else {
              // call the corresponding client method to handle the interaction
              client[consentedInteraction](username, function (err, request) {
                if (err) {
                  debug('error when interacting with ' + username +': ' + consentedInteraction);
                } else {
                  debug('successfully interacted with ' + username + ': ' + consentedInteraction);
