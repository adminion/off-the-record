...
function OffTheRecord_realtime (data, http) {
    ...
    function accountsConnect (socket) {
        ...
        socket.on('request-send', function (accountId, done) {
            user.friendRequest(accountId, done);
        });

        socket.on('request-accept', function (accountId, done) {
            user.acceptRequest(accountId, done);
        });

        socket.on('request-deny', function (accountId, done) {
            user.denyRequest(accountId, done);
        });
        ...
        socket.on('update-profile', function (updates, done) {
            ...
            data.accounts.findOneAndUpdate(user._id, updatesToUser, done);
        });
        ...
        socket.on('search', function (term, options, done) {
            ...
            data.accounts.search(user._id, term, options, done);
        });
    };
