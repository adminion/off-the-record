...
function OffTheRecord_realtime (data, http) {
  ...
  function socketConnect (socket) {
    ...
    debug('state', state);
    debug('state.users[' + socket.user.username + ']', socket.user);
    debug('state.users[' + socket.user.username + '].friends', socket.user.friends);
    ...
    socket.on('friends:get',          friendsGet);
    socket.on('friends:interactions', friendsInteractions);
    ...
    function convosStart (invitees, done) {
      ...
      invitees.forEach(function (invitee, key) {
        state.users[invitee].consents(socket.user, 'startConversation', function (err, answer) {
          if (err) return done(err);
    ...
    function friendsGet (done) {

      debug('friends:get - ' + socket.user.username);
    ...
    function friendsInteractions (username, done) {
    ...
    function friendsUnfriend (friendUsername, done) {
      ...
      if (socket.user.friends.indexOf(friend.username)) {
        ...
        socket.user.endFriendship(friend.id, function (err, result) {
          ...
          notify(socket.user.username, 'friends:unfriended', friend.username);
    ...
    function requestsAccept (requesterUsername, done) {
      ...
      data.user.getFriendship(socket.user.id, requester.id, function  (err, request) {
        ...
        socket.user.acceptRequest(requester.id, function (err, friendship) {
          ...
          debug(requester.username + '.requests', requester.friends);
    ...
    function requestsDeny (requesterUsername, done) {
      ...
      data.user.getFriendship(socket.user.id, requester.id, function  (err, request) {
        ...
        socket.user.denyRequest(requester.id, function (err, friendship) {
          if (err) return done(err);
    ...
    function requestsSend(requestedUsername, done) {
      ...
      requested.consents(socket.user.id, 'friendRequest', function (err, consent) {
        if (err) return done(err);
        ...
        socket.user.friendRequest(requested.id, function (err, request) {
          ...
          async.parallel({
            ...
            notify(socket.user.username, 'requests:sent', results.poulateRequest.requested);

            // if the requested user is online notify them too
            if (state.users[requested.username]) {
              notify(requested.username, 'requests:received', results.populateRequest.requester);
            }
  ...
  function notify (username, eventName, eventParams) {
