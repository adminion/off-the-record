...
function OffTheRecord_realtime (data, http) {
  ...
  function initSocket (socket) {
    ...
    socket.user = state.users[socket.request.user.username];

    // add the socketId to this user's cache
    state.sockets[socket.user.username].push(socket.id);
    ...
    for (var convoId in state.convos) {
      ...
      if (convo.mayJoin(socket.user.username) && !convo.isMember(socket.user.username)) {
        // join the user to the conversation
        joinConvo(socket.user.username, convoId);
      }
    }
  ...
  /////////////////////////////
    ...
    socket.on('friends:get', function (done) {
      socket.user.getFriends({ projection: 'username' }, function gotFriends (err, friends) {
        if (err) return done(err);
  ...
  function getFriendsOfFriends (username) {
    ...
    user.friends.forEach(function (friend) {
      friend.friends.forEach(function (friendOfFriend) {
        ...
        if (!~user.friends.indexOf(friendOfFriend.username)
            // and if the friendOfFriend hasn't already been found
            && !~friendsOfFriends.indexOf(friendOfFriend.username)) {
          friendsOfFriends.push(friendOfFriend.username);
        }
