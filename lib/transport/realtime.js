...
function OffTheRecord_realtime (data, http) {
    ...
    function accountsConnect (socket) {
        ...
        socket.on('update-privacy',     updatePrivacy);
         ...
         *
         */
        function search (term, options, done) {

            debug('term', term);
        
            data.search(user._id, term, options, done);

        };

        function updateProfile(updates, done) {
            ...
            data.updateAccount(user._id, updatesToUser, done);
        }

        function updatePrivacy(updates, done) {
    ...
    function convosConnect (socket) {
        ...
        // join a conversation
        socket.on('join', function joinConvo (convoId, joined) {

        });

        // leave a conversation
        socket.on('leave', function leaveConvo (convoId, left) {
