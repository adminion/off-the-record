        ...
        if (!request.params.email || request.params.email === request.user.email) { 
            ...
            data.getAccountByEmail(email, '_id created email profile privacy', function (err, account) {
                ...
                if (err) {
                    ...
                    data.getRelationship(request.user._id, account._id, function (err, relationship) {
                        if (err) {
                            fiveHundred(err, request, response);
                        } 
                        ...
                        if (relationship === data.FRIENDS) {
                            accountInfo.created = account.created;
                            accountInfo.profile = account.profile;
                            accountInfo.privacy = account.privacy;
                        }
