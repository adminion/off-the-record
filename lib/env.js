...
if (scriptStats.isSymbolicLink()) {
  ...
  console.log('scriptStats.isSymbolicLink() === true');

  linkTarget = fs.readlinkSync(script);
} 

// use readlink to figure out the file to which it links
scriptPath = path.resolve(script, linkTarget);

console.log('scriptPath', scriptPath);

// the prefix is the absolute path to the directory that server.js is in
...
bannerLines = fs.readFileSync(__dirname + '../banner.txt', {encoding: 'utf8' }).trim().split('\n');
