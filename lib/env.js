...
var debug       = require('debug')(pkg.name + ':env');

var env = {};

env.prefix = process.env.OTR_PREFIX || path.resolve(__dirname, '..');
...
env.url = function () {
  ...
  let urlFormat = 'https://%s:%s';
  let url = (config.http.host) 
    ? util.format(urlFormat, config.http.host, this.net.port)
    : util.format(urlFormat, this.net.addresses[0], this.net.port);
